...
const MovieModal = ({ isOpen, onClose, editingMovie, formData, errors, onChange, onSubmit }) => (
  <Modal isOpen={isOpen} onClose={onClose} title={editingMovie ? 'Cập nhật phim' : 'Thêm phim mới'}>
    <form onSubmit={onSubmit} className="space-y-6 max-h-[75vh] overflow-y-auto pr-2 no-scrollbar">
      {errors?.submit && (
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" name="HinhAnh" placeholder="Nhập link URL hình ảnh poster..."
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-xs text-white"
          ...
          <div className="space-y-2">
            ...
            <select name="KhaDung" value={formData.KhaDung} onChange={onChange}
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-sm text-slate-300">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <input type="text" name="TenPhim" required placeholder="VD: Lật Mặt 7"
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-sm font-bold text-white"
            ...
            <div className="space-y-1">
              ...
              <input type="number" name="ThoiLuong" required min="1" placeholder="120"
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-sm text-white"
            ...
            <div className="space-y-1">
              ...
              <select name="GioiHanTuoi" value={formData.GioiHanTuoi} onChange={onChange}
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-sm text-slate-300">
                {['P', 'C13', 'C16', 'C18'].map(g => (
                  <option key={g} value={g} className="bg-[#0f1117]">{g}</option>
                ))}
              </select>
            </div>
            {/* Trailer */}
          ...
          <div className="space-y-1">
            ...
            <textarea name="NoiDung" rows={4} placeholder="Tóm tắt cốt truyện..."
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-red-500 text-sm resize-none text-white"
