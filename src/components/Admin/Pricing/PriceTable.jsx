...
const PriceTable = ({ title, data, typeKey, nameKey, onAdd, onEdit, onDelete }) => {
  ...
  return (
    <><div className="admin-legacy-table bg-[#131A2A]/40 backdrop-blur-md border border-white/[0.06] rounded-3xl overflow-hidden shadow-2xl flex flex-col w-full hover:border-white/10 transition-all duration-300">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-white/[0.06] bg-white/[0.01] select-none whitespace-nowrap">
                  {['Tên hiển thị', 'Giá phụ thu', 'Mô tả', 'Trạng thái', 'Ngày tạo', 'Cập nhật', 'Hành động'].map(h => (
    ...
    </div><AdminDetailDialog item={detailItem} title={detailItem?.[nameKey] || 'Chi tiết bảng giá'} onClose={() => setDetailItem(null)} /></>
  );
};
