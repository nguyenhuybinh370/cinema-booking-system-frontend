...
const TransactionHistory = () => {
  ...
  const [activeTab, setActiveTab] = useState(location.state?.defaultTab || "ban-ve");
  ...
  return (
    <div className="flex flex-col h-full space-y-6 py-2">
      ...
      <div className="flex border-b border-white/5 shrink-0 gap-6">
        {[
          { id: "ban-ve", label: "Lịch sử bán vé" },
          { id: "soat-ve", label: "Lịch sử soát vé" },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => switchTab(tab.id)}
            className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all relative cursor-pointer ${
              activeTab === tab.id ? "text-[#FFB000]" : "text-slate-400 hover:text-white"
            }`}
          >
            {tab.label}
            {activeTab === tab.id && (
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#FFB000] shadow-[0_0_8px_#FFB000]" />
            )}
          </button>
        ))}
      </div>
      ...
      <div className="bg-[#131A2A]/80 border border-white/[0.06] shadow-xl rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center shrink-0">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          <input
            ...
            className="w-full md:w-44 bg-slate-950/40 border border-white/5 focus:border-[#FFB000] rounded-xl py-2.5 px-3 text-xs text-white/70 focus:outline-none"
            title="Từ ngày"
          />
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                logs.map((log) =>
                  activeTab === "soat-ve" ? (
                    <CheckInHistoryRow key={log.MaChiTietDat} log={log} />
                  ) : (
                    <SaleHistoryRow key={log.MaPhieuDat} log={log} />
                  )
                )
              )}
            </tbody>
